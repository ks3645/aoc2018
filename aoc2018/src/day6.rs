...
fn part_two(input: String) -> i32 {
    let points: Vec<(usize, Point)> = (1..)
        .zip(input.lines().map(|line| Point::from(line)))
        .collect();
    ...
    for x in start_x - (width * 2)..end_x + (width * 2) {
        for y in start_y - (height * 2)..end_y + (height * 2) {
            ...
            let mut dists: Vec<(usize, i32)> =
                points.iter().map(|(id, p)| (*id, p.dist_to(&pt))).collect();
